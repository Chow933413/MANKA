...
# NEW: Clean toggle flag to manage locked level gates!
...
func _ready() -> void:
	...
	# Automatically handle its own layout state
	if is_locked:
		is_active = false
		hide()
	else:
...
func unlock() -> void:
	is_active = true
	show()
	print("Portal ", portal_id, " has been completely unlocked and revealed!")
...
func _on_body_entered(body: Node3D) -> void:
	...
	if body.name == "Player" or body.is_in_group("player"):
		...
		if target_scene:
			FadeTransition.fade_to_next_scene(target_scene)
